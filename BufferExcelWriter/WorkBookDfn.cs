...
namespace BufferExcelWriter
{
    public class WorkBookDfn : IDisposable
    {
        ...
        public void FlushBufferedRows(bool needGc = false)
        {
            UpdateSheetNum();
            foreach (var sheet in Sheets)
            {
                ...
                foreach (var currentRow in sheet.BufferedRows)
                {
                    sheet.TempDataStreamWriter.WriteLine(JsonConvert.SerializeObject(currentRow));
                }

                sheet.TempDataStreamWriter.Flush();
                sheet.BufferedRows.Clear();
            }
        ...
        private void UpdateSheetRelationship()
        {
            InitFromZipFile();
            UpdateSheetNum();
        ...
        public Stream BuildExcelAndGetStream(bool needGc = true)
        {
            if (Sheets.Count == 0)
            {
                throw new InvalidOperationException("no sheet in workbook");
            ...
            FlushBufferedRows(needGc);
            UpdateSheetRelationship();

            foreach (var sheet in Sheets)
            {
                GenerateSheetFile(sheet);
            }
        ...
        private void GenerateSheetFile(WorkSheetDfn sheet)
        {
            var sheetEntry = FolderEntry.GetEntry(sheet.GetEntryName());
            if (sheetEntry == null)
            {
            ...
            using (sheet.SheetStreamWriter = new StreamWriter(sheet.SheetFileStream))
            {
                sheet.SheetStreamWriter.Write(WorksheetDefaultHeaders);
                var rowNumber = 1;
                sheet.SheetStreamWriter.Write(sheet.Header.ToXmlString(rowNumber, sheet.Header,
                    sheet.NullValStr));
                if (sheet.TempDataStream != null)
                {
                    sheet.TempDataStream.Seek(0, SeekOrigin.Begin);
                    using (var streamReader = new StreamReader(sheet.TempDataStream))
                    {
                        while (!streamReader.EndOfStream)
                        {
                            ...
                            var currentRow = JsonConvert.DeserializeObject<RowDfn>(line);
                            sheet.SheetStreamWriter.Write(currentRow.ToXmlString(rowNumber,
                                sheet.Header,
                                sheet.NullValStr));
                        }
                    }
                ...
                sheet.SheetStreamWriter.Write(SheetDataDefaultFooter);
                sheet.SheetStreamWriter.Write(WorksheetDefaultFooter);
                sheet.SheetStreamWriter.Flush();
            }
        }
